...
## 🌐 Decentralized Community Threat Vault (Phase 5)

> [!NOTE]
> Based on the user's request, we are transitioning the localized Merkle Threat Vault into a **Decentralized, Crowdsourced Threat Vault**. A central website/backend will connect all extension users to pool their ML detections.

### Architecture
**1. Web App & API Server (The Database)**
- We will build a backend application (e.g., using Node.js/Express or Next.js) that serves as the central ledger node and the public marketing website for the extension.
- It will expose API endpoints (e.g., `/api/vault/sync` and `/api/vault/submit`).

**2. Global Merkle Tree**
- The server maintains the master Merkle Tree of all community-blocked domain hashes.
- Instead of relying entirely on initial developer datasets, the database grows organically as users encounter novel zero-day threats.

**3. Client Synchronization ([background.js](file:///d:/Browser-Vigilant/background.js))**
- On browser startup, the extension syncs with the server to download the latest threat hashes (or only the differential blocks it's missing).
- **Proactive Reporting**: When the local ONNX ML model detects a new threat with high confidence (e.g., > 95%), the extension hashes the domain and submits it to the backend. Lookups remain O(1) in memory.

**4. Privacy Preservation**
- Clients **NEVER** transmit full URLs or browsing data to the community database.
- Only the irreversible `SHA-256(hostname)` is broadcasted. This ensures strict zero-telemetry while allowing everyone to benefit from shared real-time ML detections.

---

## 🛡️ Advanced Active Firewall & Deep ML Architecture (Phase 6)

To elevate Browser Vigilant from a passive scanner to an **Advanced Active Firewall**, we will implement the following state-of-the-art capabilities:

### 1. Active WebRequest Firewall (Layer 0)
- **Deep Packet Inspection (DPI) in Browser**: We will utilize `chrome.webRequest` and `chrome.declarativeNetRequest` to inspect not just the top-level URL, but **every sub-resource** (XHR, Fetch, WebSockets, Images, Scripts).
- **Data Exfiltration Blocking**: We will detect and block outbound anomalous payloads (e.g., intercepting an XHR POST containing base64-encoded strings that look like passwords or session tokens sent to an untrusted domain).
- **Third-Party Tracker & C2 Blocking**: Automatically sever connections to known Command & Control (C2) servers or malicious ad networks injecting malvertising.

### 2. Deep UPI & Payment Fraud Detection
- **Contextual DOM NLP**: Instead of just looking for `upi://pay`, the extension will analyze the surrounding DOM text using lightweight on-device NLP rules to detect urgency/scam context (e.g., "Pay Rs 10 to receive Rs 1000 cashback" + a UPI link).
- **Invisible Overlay Triangulation**: We will compute the z-index, opacity, and click-interception events of payment gateways. If a transparent `<iframe>` or `<div>` is layered exactly over a legitimate bank's "Submit" button to steal the click, the transaction is physically blocked on the DOM level.

### 3. "Active Learning" ML Backend
- **Anti-Poisoning Consensus**: If we just accept any hash from any user, a bad actor could submit `SHA-256("google.com")` and break the internet for everyone.
- **Server-Side Verification Engine**: When a client submits a new zero-day threat hash, the backend **does not** immediately broadcast it.
  - The backend decrypts the report (clients will send the raw URL encrypted for backend eyes only, or the backend crawls the reported hash if possible).
  - The backend runs a heavy Server-Side Deep Learning agent (e.g., a Headless Chromium cluster running DOM analysis + LLM vision models) to independently verify the threat.
  - Only upon server consensus is the hash signed via cryptography and distributed to the global Merkle Tree.
- **Dynamic Feature Weights**: The backend will continuously train on the aggregated global telemetry and push new optimized ONNX weights ([model.onnx](file:///d:/Browser-Vigilant/model.onnx) v2, v3, etc.) natively to clients via the sync API, creating a self-healing, self-advancing neural network.
...
- `http://paypal-secure.account-verify.xyz` → blocked
